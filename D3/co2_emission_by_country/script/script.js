export const barChart = (data, {
    ...
    const filter_data = data.filter(function(d) {return (d.year == year && d.continent == continent)});
    const X = d3.map(filter_data, x);
    const Xint = X.map(function(d) {return +d;});
    const Y = d3.map(filter_data, y);

    const xDomain = [0, d3.max(Xint)+2];
    const yDomain = new d3.InternSet(Y);

    const height =  500 - marginTop - marginBottom; //Math.ceil((yDomain.size + yPadding)*25) + marginTop + marginBottom;
